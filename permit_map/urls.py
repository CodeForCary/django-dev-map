...
urlpatterns = [
	...
	url(r'^permits/all', cache_page(2592000)(GeoJSONLayerView.as_view(model=models.PermitArea, geometry_field='region', 
		properties=['category', 'id', 'township', 'first_seen', 'last_seen'])), name='all'),
	# Full text search entry point, delegates to views.search (views.py)
	# Permit lookup point, delegates to views.permitsat (views.py)
	url(r'^permits/search', views.search, name='search'),
	url(r'^permits/at', views.permitsat, name='at'),
	url(r'^permits/overview', views.overview, name='overview')
]
